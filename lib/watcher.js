...
class Watcher extends EventEmitter {
    ...
    watch() {
        this.watcher = fs.watch(this.dir, {
             ...
             *      }
             */
            this.emit('watcher:' + eventType, { dir: this.dir, filename });
        });
    ...
    read() {
        ...
        recursive(this.dir, [(f, s) => {
            // ignore because?
            return path.basename(f).substr(0, 1) == '.'; // skip files starting with a dot, because OSX
        }], (err, files) => {
            if(err) {
            ...
            for( const f of files ) {
                ...
                this.emit('watcher:read', { dir: this.dir, filename });
            }
        });
